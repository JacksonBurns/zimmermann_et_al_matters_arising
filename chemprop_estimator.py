# copied from https://github.com/jxl26/chemprop/blob/da0af4664397e10811be4d3f325c23726e806538/chemprop/sklearn_integration/chemprop_estimator.py
...
class ChempropRegressor(BaseEstimator, RegressorMixin):
    def __init__(
        ...
        self.model = None
        patience = (
            self.args.patience if self.args.patience is not None else self.args.epochs
        )
        self.trainer = Trainer(
            ...
            callbacks=[
                EarlyStopping(monitor="train_loss", patience=patience, mode="min")
            ],
        )
    ...
    def predict(self, X: MoleculeDataset | ReactionDataset | MulticomponentDataset):
        ...
        preds = trainer.predict(self.model, dataloaders=dl)
...
if __name__ == "__main__":
    ...
    sklearnPipeline = Pipeline(
        [
            ...
            (
                ...
                ChempropRegressor(
                    epochs=10,
                    checkpoint=[Path("example_model_v2_regression_rxn+mol.pt")],
                ),
            ),
        ]
    )
    ...
    score = sklearnPipeline.score(
        X=Path("../../tests/data/regression/rxn+mol/rxn+mol.csv")
    )
    print(f"RMSE: {score}")
